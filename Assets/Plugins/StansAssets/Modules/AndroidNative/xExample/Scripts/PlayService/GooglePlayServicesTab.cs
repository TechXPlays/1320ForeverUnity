...
public class GooglePlayServicesTab : FeatureTab {
	...
	//example
	private const string LEADERBOARD_NAME = "leaderboard_most_wanted";
	...
	//example
	private const string LEADERBOARD_ID = "CgkI2Pz9ib8FEAIQBg";
	//private const string LEADERBOARD_ID = "REPLACE_WITH_YOUR_ID";
	
	
	
	private const string INCREMENTAL_ACHIEVEMENT_ID = "CgkI2Pz9ib8FEAIQBw";
	...
	private void OnScoreSubmited(GP_LeaderboardResult result) {
		if (result.IsSucceeded) {
			...
			Debug.Log ("result.Response " + result.Response);
			Debug.Log ("result.Message " + result.Message);
			Debug.Log ("result.Leaderboard.id "  + result.Leaderboard.Id);
